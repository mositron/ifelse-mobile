...
class ProductState extends State<ProductPage> {
  ...
  int id = 0,
      ...
      stock = 0,
      stocked = 0;
  String title = '', eachName1 = '', eachName2 = '', unit = '', type = '';
  Map image;
  double weight = 0;
  List<double> price = [0, 0];
  List<dynamic> items = [], style1 = [], style2 = [];
          ...
          return FutureBuilder<Map<dynamic, dynamic>>(
              ...
              builder: (context, snapshot) {
                return snapshot.connectionState == ConnectionState.done
                    ? snapshot.hasData ? getWidget(snapshot.data) : Product.retryButton(fetch)
                    : IfDialog.getLoading();
              });
        } else {
          return Container();
  ...
  Widget getWidget(dynamic data) {
    if (data is Map) {
      ...
      stocked = getInt(data['stocked']);
      type = getString(data['type']);
      if ((type != 'download') && (type != 'service') && (type != 'parcel')) {
        type = 'parcel';
      }
      diff = getInt(data['diff']);
      dynamic dim = getVal(data, 'dim');
      if ((dim == null) || (dim is! Map)) {
        dim = {};
      }
      ...
      return BlocProvider<ProductBloc>(create: (context) => productBloc, child: Layer.buildContent('product', context, data, _click));
    }
    return Container();
  }

  void _click(String event, [Map data]) {
    if (event == 'button') {
      Map item = {
        'id': id,
        'title': title,
      ...
      if (data['type'] == 'inc') {
        ...
        if (type == 'download') {
          ...
          if ((stocked > 0) && (amount > stock)) {
            amount = stock;
            Toast.show('มีจำนวนสินค้าชนิดนี้ในคลัง ' + amount.toString() + ' ' + unit, context, duration: Toast.lengthLong, gravity: Toast.bottom);
          }
        } else {
          if ((eachStyle == 1) && (eachStyle1 > -1)) {
            if ((stocked > 0) && (amount > items[eachStyle1]['stock'])) {
              amount = items[eachStyle1]['stock'];
            }
          } else if ((eachStyle == 2) && (eachStyle1 > -1) && (eachStyle2 > -1)) {
            index = (eachStyle1 * eachCount2) + eachStyle2;
            if ((stocked > 0) && (amount > items[index]['stock'])) {
              amount = items[index]['stock'];
            }
          }
        }
        productBloc.add({'event': 'amount', 'value': amount});
        ...
        if (eachStyle == 1) {
          productBloc.add({
            'event': 'update',
            'value': {'style1': eachStyle1, 'price': items[eachStyle1]['price'], 'stock': items[eachStyle1]['stock'], 'unit': unit}
          });
        } else if (eachStyle2 > -1) {
          index = (eachStyle1 * eachCount2) + eachStyle2;
          productBloc.add({
            'event': 'update',
            'value': {'style1': eachStyle1, 'style2': eachStyle2, 'price': items[index]['price'], 'stock': items[index]['stock'], 'unit': unit}
          });
        } else {
          productBloc.add({'event': 'style1', 'value': eachStyle1});
        }
        ...
        if (eachStyle1 > -1) {
          ...
          productBloc.add({'event': 'style2', 'value': eachStyle2});
        }
      }
    }
  }
