...
class CheckoutState extends State<CheckoutPage> {
          ...
          return FutureBuilder<Map<dynamic, dynamic>>(
              ...
              builder: (context, snapshot) {
                return snapshot.connectionState == ConnectionState.done
                    ? snapshot.hasData ? getWidget(snapshot.data) : retryButton(fetch)
                    : IfDialog.getLoading();
              });
        });
  }
                            ...
                            BlocBuilder<CheckoutBloc, int>(
                                ...
                                builder: (_, count) {
                                  if ((addressSelected == -1) || (addressSelected >= Cart.address.length)) {
                                    return Text('กรุณาเลือกที่อยู่ในการจัดส่ง',
                                        style: TextStyle(fontFamily: Site.font, color: Colors.redAccent, fontSize: 16));
                                  } else if ((shippingSelected == -1) || (shippingSelected >= shipping.length)) {
                                    return Text('กรุณาเลือกวิธีการจัดส่ง',
                                        style: TextStyle(fontFamily: Site.font, color: Colors.redAccent, fontSize: 16));
                                  } else {
                                    return Text(getCurrency(Cart.price + Cart.shipFee),
                                        style: TextStyle(fontFamily: Site.font, color: Color(0xffff5717), fontSize: 18));
                                  }
                                })
                          ],
                        )),
  ...
  Widget selectedAddressSection() {
    if ((Cart.address != null) && (Cart.address is List)) {
      return Container(
        decoration:
            BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(5)), border: Border.all(color: Colors.grey.shade200), color: Colors.white),
                                    ...
                                    return AlertDialog(
                                      ...
                                      actions: <Widget>[
                                        ...
                                        FlatButton(
                                          ...
                                          child: Text('ยกเลิก',
                                              style: TextStyle(fontFamily: Site.font, fontSize: Site.fontSize, color: Color(0xffff5717))),
                                        ),
                                      ],
                                    );
                                  },
                                  ...
                                  child: Card(
                                      ...
                                      child: Container(
                                          ...
                                          child: Row(children: [
                                            ...
                                            Expanded(
                                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                              Text(getString(item['name']) + ' (' + getString(item['phone']) + ')',
                                                  style: TextStyle(fontFamily: Site.font), maxLines: 1),
    ...
    return Container(child: Text('- ยังไม่มีที่อยู่ในการส่ง -'));
  }
  ...
  standardDelivery() {
                    ...
                    Container(
                        child: ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, int position) {
                              ...
                              final double shipFee = calcShipping(item);
                              _click() {
                                                ...
                                                children: [
                                                  ...
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      (shipFee > 0
                                                          ? Text('+' + getCurrency(shipFee),
                                                              style: TextStyle(fontFamily: Site.font, color: Color(0xffff5717)),
                                                              textAlign: TextAlign.right)
                                                          : Text('ฟรี',
                                                              style: TextStyle(fontFamily: Site.font, color: Colors.green),
                                                              textAlign: TextAlign.right))
                                                    ],
                                                  )
                                                ],
    ...
    return Container(child: Text('- ยังไม่มีวิธีการส่ง (กรุณาติดต่อเจ้าของเว็บไซต์) -'));
  }
  ...
  double calcShipping(Map item) {
    ...
    if (rate == 3) {
      ...
      print('shipWeight');
      print(Cart.shipWeight);
      if (Cart.shipWeight > 0) {
        dynamic range = item['range'];
        if ((range != null) && (range is Map)) {
          for (var key in range.keys) {
            double weight = getDouble(key);
            if (Cart.shipWeight < weight) {
              found = getDouble(range[key]);
              break;
            }
          }
      ...
      print('shipAmount');
      print(Cart.shipAmount);
      if (Cart.shipAmount > 0) {
        dynamic range = item['range'];
        if ((range != null) && (range is Map)) {
          for (var key in range.keys) {
            double amount = getDouble(key);
            if (Cart.shipAmount < amount) {
              found = getDouble(range[key]);
              break;
            }
          }
      ...
      print(Cart.shipAmount);
      if (Cart.shipAmount > 0) {
        found = getDouble(item['range']) + ((Cart.shipAmount - 1) * getDouble(item['price']));
      } else {
        found = 0;
      }
    }
  ...
  Widget checkoutListItem(Map item) {
    if ((item != null) && (item is Map)) {
      ...
      List<Widget> lines = [];
      lines.add(Text(title, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: Site.font, fontSize: Site.fontSize)));
      lines.add(SizedBox(height: 5));
      if (label.length > 0) {
        lines.add(Text(label.join(' - '), maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: Site.font, fontSize: 14)));
        lines.add(SizedBox(height: 5));
      }
      lines.add(Container(
        child: Text(getCurrency(price) + ' x ' + amount.toString(),
            maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: Site.font)),
      ));
      ...
      return Container(
        ...
        child: SizedBox(
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              Container(width: 90, child: getImageRatio(image, 's', 0)),
              SizedBox(width: 15),
              Expanded(
                flex: 1,
                child: Column(
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      Cart.shipFee > 0 ? getCurrency(Cart.shipFee) : 'ฟรี',
                      style: TextStyle(fontFamily: Site.font),
                    )
                  ],
                ),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      getCurrency(Cart.price + Cart.shipFee),
                      style: TextStyle(fontFamily: Site.font),
                    )
                  ],
                ),
