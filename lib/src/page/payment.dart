...
class PaymentState extends State<PaymentPage> {
  ...
  Widget paymentBank(dynamic data) {
                                  ...
                                  child: Container(
                                      ...
                                      child: Row(children: [
                                        ...
                                        Expanded(
                                          ...
                                          Text(Bank.getName(item['bank']) + ': ' + getString(item['number']),
                                              style: TextStyle(fontFamily: Site.font), maxLines: 2),
                                        ]))
                                      ])));
                            }));
                  }))
    ...
    return Container(child: Text('- ยังไม่มีวิธีการชำระเงิน -'));
  }
  ...
  _sendCart() async {
    ...
    if (error.length > 0) {
      ...
      dynamic response = await Api.call('cart', {
        ...
        'weight': Cart.shipWeight.toString(),
        'ship_id': Cart.shipId.toString(),
        'ship_price': Cart.shipFee.toString(),
      ...
      if ((response != null) && (response is Map)) {
        if ((response['error'] != null) && (response['error'] is List)) {
          ...
          response['error'].forEach((v) {
            _error.add(
                Text(getString(v), maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: Site.font, fontSize: Site.fontSize)));
          });
          if (_error.length > 0) {
            showDialog(
                context: context,
