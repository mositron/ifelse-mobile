...
class Cart {
  ...
  static int shipId = 0;
  static double shipFee = 0;
  static double shipPrice = 0;
  static double shipWeight = 0;
  static int shipAmount = 0;
  ...
  static void refresh([bool save = true]) {
    ...
    if (products.length > 0) {
      for (int i = 0; i < products.length; i++) {
        if ((products[i] != null) && (products[i] is Map) && (products[i]['amount'] != null) && (products[i]['amount'] is int)) {
          if ((getInt(products[i]['stocked']) > 0) && (products[i]['amount'] > products[i]['stock'])) {
            products[i]['amount'] = products[i]['stock'];
          }
          _amount += products[i]['amount'];
          if (products[i]['type'] == 'parcel') {
            _shipAmount += products[i]['amount'];
          }
        }
      }
    }
    ...
    Cart.shipAmount = _shipAmount;
    Cart.shipWeight = getWeight();
    Cart.shipPrice = getShipPrice();
    if (save) {
  ...
  static double getShipPrice() {
    double sumPrice = 0;
    if ((products != null) && (products is List) && (products.length > 0)) {
      products.forEach((v) {
        int _amount = getInt(v['amount']);
        double _price = getDouble(v['price']);
        String _type = getString(v['type']);
        print(_type);
        if ((_type == 'parcel') && (_amount > 0) && (_price > 0)) {
          sumPrice += (_amount * _price);
        }
      });
    }
  ...
  static double getWeight() {
    ...
    if ((products != null) && (products is List) && (products.length > 0)) {
      products.forEach((v) {
        ...
        String _type = getString(v['type']);
        if ((_type == 'parcel') && (_amount > 0) && (_weight > 0)) {
...
class _CartCellState extends State<CartCell> {
  ...
  Color color;
  double fsize;
  _CartCellState(this.id, this.index, this.title, this.image, this.amount, this.price, this.name1, this.label1, this.name2, this.label2, this.unit,
      this.stock, this.color, this.fsize);
              ...
              child: RawMaterialButton(
                onPressed: () {
                  setState(() {
                    ...
                    if (amount < 1) {
                      amount = 1;
                      Toast.show('หากต้องการลบ ให้ปัดสินค้ารายการนี้ไปทางด้านซ้ายหรือขวา', context,
              ...
              child: RawMaterialButton(
                onPressed: () {
                  setState(() {
                    ...
                    if (amount > stock) {
                      amount = stock;
                      Toast.show('มีสินค้านี้คงเหลือในคลัง ' + stock.toString() + ' ' + unit, context,
                          duration: Toast.lengthLong, gravity: Toast.bottom);
                    }
                    if (Cart.products.length > 0) {
                      for (int i = 0; i < Cart.products.length; i++) {
    ...
    return Container(
      ...
      child: SizedBox(
        child: Row(
          ...
          children: <Widget>[
            Container(width: 70, child: getImageRatio(image, 's', 0)),
            SizedBox(width: 10),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
