...
class SpecParser extends WidgetParser {
  Widget parse(String file, Map<String, dynamic> map, BuildContext buildContext,
      [Map<String, dynamic> par, Function func]) {
    return new SpecView(
        key: UniqueKey(),
        map: map,
...
class SpecView extends StatefulWidget {
  ...
  final dynamic par;
  final Function func;
  SpecView({Key key, this.map, this.buildContext, this.par, this.func})
      : super(key: key);
...
class _SpecViewState extends State<SpecView> {
  ...
  Widget build(BuildContext context) {
    dynamic box = getVal(_map, 'box'), data = getVal(_map, 'data');
      ...
      if (eachStyle > 0) {
        ...
        if ((style1Item != null) &&
          ...
          style1Item.forEach((val) {
            if (val is Map) {
              final j = i;
              _btn.add(BlocBuilder<ProductBloc, Map>(
                  ...
                  builder: (_, product) {
                    return RaisedButton(
                      ...
                      color: product['style1'] == j
                          ? Color(0xffe7e7e7)
                          : Colors.white,
                      child: Text(getString(val['name']),
          ...
          Container(
              ...
              child: Row(children: [
                ConstrainedBox(
                    ...
                    child: Text(eachName1,
                        style: TextStyle(
                            fontFamily: Site.font,
                            fontSize:
                                getDouble(getVal(data, 'size'), Site.fontSize),
                            color: getColor(getVal(data, 'color'))))),
                ...
                builder: (_, product) {
                  ...
                  if (product['stocked'] > 0) {
                    if (eachStyle == 1) {
                      if (product['style1'] > -1) {
                        if (product['stock'] > 0) {
                          txt = 'จำนวนสินค้าในคลัง ' +
                              getString(product['stock']) +
                              ' ' +
        ...
        children: [
          ...
          Container(
              ...
              child: BlocBuilder<ProductBloc, Map>(
                  bloc: context.repository<ProductBloc>(),
                  builder: (_, product) {
                    return Text(product['amount'].toString(),
                        style: TextStyle(
                            fontFamily: Site.font, color: Colors.black));
                  })),
          SizedBox(
            width: 30,
            height: 30,
